...
**Migrations:** With **`docker compose`**, the **`db-migrate`** service runs **Liquibase** against **`db/changelog/`** before the **`app`** container starts when **`EXAMINAI_USE_LIQUIBASE`** is set on **`app`** (the Python **Dockerfile** does not include a JVM). Changelogs live under **`db/changelog/`** (baseline: `001-baseline.postgresql.sql`). **Do not** duplicate that DDL in Postgres `docker-entrypoint-initdb.d` — init scripts there are for optional hooks only; **Liquibase owns DDL** in that deployment mode. Developers may also run **`python -m examai.liquibase_cli`** if the Liquibase CLI is installed locally. For local development on **SQLite** or Postgres without Liquibase, the app may still use SQLAlchemy **`create_all`** (see `examai.database.create_schema`). **Alembic** remains available in **`pyproject.toml`** for a future single migration pipeline; until Alembic revisions exist, treat **Liquibase** as the source of truth for containerized PostgreSQL schema.
